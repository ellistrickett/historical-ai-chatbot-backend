...
export async function createChat(req, res, next) {
  try {
    ...
    // 1. Validation (Keep basic input validation in Controller)
    if (!personaName || !messages) {
      return res.status(400).json({ error: 'PersonaName and messages are required.' });
    }

    // 2. Delegate Logic to Service (Service generates ID and Date)
    ...
    // 3. Return the ID generated by the service
    res.status(201).json({
